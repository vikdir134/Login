...
import { listOrdersInProcess } from '../api/orders'
...
export default function PedidosProceso() {
  ...
  const load = async () => {
    ...
    try {
      const data = await listOrdersInProcess({ q, limit: pageSize, offset: page*pageSize })
      setRows(Array.isArray(data?.items) ? data.items : [])
      setTotal(Number(data?.total || 0))
    } catch (e) {
      console.error(e); setMsg('Error cargando pedidos en proceso')
  ...
  return (
    <section className="card">
      <div className="topbar" style={{ marginBottom:0 }}>
        <h3 style={{ margin:0 }}>Selecciona un pedido en proceso</h3>
      </div>
      ...
      <div className="table" style={{ marginTop:14 }}>
        ...
        {!loading && rows.map(r=>(
          <div className="table__row" key={r.id} style={{ gridTemplateColumns:'1fr 2fr 1fr auto' }}>
            ...
            <div><span className="badge">{r.state}</span></div>
            <div>
        ...
        {!loading && rows.length===0 && <div className="muted">No hay pedidos en proceso</div>}
      </div>
